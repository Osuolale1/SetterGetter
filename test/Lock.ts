...
import { SetterGetter__factory } from "../typechain-types";
...
describe("Setter Getter", function () {
  ...
  describe("Setting", function () {
    it("Should be able to set the name", async function () {
      const { setterGetter } = await loadFixture(deploySetterGetter);

      const tx = await setterGetter.setNam("femi");

      const name = await setterGetter.getName();

      expect(name).to.equal("femi");
    });
  });
  });
